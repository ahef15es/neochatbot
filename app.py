...
Settings.node_parser = SentenceSplitter(chunk_size=4096, chunk_overlap=1024)
...
context = (
    ...
    "REPLY ALWAYS IN THE LANGUAGE IN WHICH THE QUESTION WAS CREATED"
...
chat_engine = index.as_chat_engine(chat_mode= "context", system_prompt=system_prompt, context_template=context)
...
def response(message, history,session_id):
    ...
    except RuntimeError:  # No loop currently running
    ...
    response_content = history + [(message, output_text)]
            ...
            with gr.Row():  
                with gr.Column(scale=5): 
                    with gr.Blocks(title="Neo Skills Chatbot", theme=theme, css="chatinterface.css"):
                        ...
                        placeholder_hmtl = "<strong>Haben Sie irgendwelche Fragen? Wie kann ich Ihnen helfen?</strong><p>Folgendes wären Beispielfragen: <ul><li>Was ist der Bereich CREATE</li><li>Worum geht es im Bereich DIGITAL WORLD</li></p>"

                        chatbot = gr.Chatbot( height= 850, label=None, show_label=False, placeholder=placeholder_hmtl)
